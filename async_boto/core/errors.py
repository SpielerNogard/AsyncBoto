...
class ErrorFactory:
    ...
    def create_exception(name, message, api_response):
        return type(name, (APIResponseException,), {
            "__init__": lambda self, msg=message: super(self.__class__, self).__init__(
                msg),
        "message": message,"error_type": name, "response": api_response})
